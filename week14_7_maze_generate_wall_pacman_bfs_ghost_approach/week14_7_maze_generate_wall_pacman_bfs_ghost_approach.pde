...
boolean ghostTestMoveAndMove(int i0, int j0, int i, int j){
  if(i<0 || j<0 || i>=10 || j>=10) return false;
  if( (i0==i && wallH[i][min(j0,j)]==1) || (j0==j && wallW[min(i0,i)][j]==1) ) {
    if(yuutaDist[i0][j0] > yuutaDist[i][j]) {
      ghostI = i;
      ghostJ = j;
      return true;
    }
...
void mousePressed(){
  if(ghostTestMoveAndMove(ghostI, ghostJ, ghostI, ghostJ+1)) return;
  if(ghostTestMoveAndMove(ghostI, ghostJ, ghostI, ghostJ-1)) return;
  if(ghostTestMoveAndMove(ghostI, ghostJ, ghostI+1, ghostJ)) return;
  if(ghostTestMoveAndMove(ghostI, ghostJ, ghostI-1, ghostJ)) return;
}
